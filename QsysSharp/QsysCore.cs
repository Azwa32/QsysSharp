...
using QsysSharp.Communications.Sockets;
using QsysSharp.Intermediaries;
...
namespace QsysSharp
{
    ...
    public class QsysCore : IDisposable
    {
        ...
        private void AddComponentToChangeGroup(NamedComponent component)
        {
            ...
            var addComponent = QsysSharp.AddComponentToChangeGroup.Instantiate(component.ToComponentSubscribeControls());
        ...
        {
            var addControls = QsysSharp.AddControlToChangeGroup.Instantiate(controls.Where(c => c.Subscribe).Select(c => c.Name));
